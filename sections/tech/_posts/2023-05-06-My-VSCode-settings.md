...
````json
{
	...
	// Defines where `black` is imported from. This setting may be ignored if `black-formatter.path` is set.
	//  - useBundled: Always use the bundled version of `black`.
	//  - fromEnvironment: Use `black` from environment, fallback to bundled version only if `black` not available in the environment.
	...
	"black-formatter.logLevel": "error",

	// When set to a path to `black` binary, extension will use that for linting. NOTE: Using this option may slowdown linting.
	...
	"python.defaultInterpreterPath": "/Users/carlo/opt/miniconda3/envs/callimacus/bin/python",
	...
	"jupyter.interactiveWindow.cellMarker.decorateCells": true,
