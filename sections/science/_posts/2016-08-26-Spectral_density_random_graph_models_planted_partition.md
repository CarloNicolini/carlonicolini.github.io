...
Among the many graph measures that I think are useful for the comparison of two different networks, in the last period, a particular one is emerging that is based on **graph spectra**. It turned out indeed, that the empirical distribution of the eigenvalues of adjacency matrices of graphs, can tell us more about the general properties of the network than some other aggregate measures, like for example the degree distribution, or other metrics.

Moreover, a deep parallel connection exists between complex networks and quantum physics and recently this connection is under investigation by researchers in complex networks, mathematics and physics. 

### Graph eigen-things
We know that the spectrum of a matrix is the set of all its eigenvalues. The eigendecomposition of a real $$n\times n$$ matrix $$\mathbf{A}$$ is the following:

$$
\mathbf{A} = \mathbf{Q} \mathbf{\Lambda} \mathbf{Q}^{-1}
$$

where $$\mathbf{Q}$$ is the eigenvectors matrix and $$\mathbf{\Lambda}$$ is the eigenvalues diagonal matrix. One can then take the diagonal elements of the matrix $$\mathbf{\Lambda}$$, i.e. the set $$\{\lambda_1, \ldots \lambda_n\}$$ and get its empirical distribution.

 In fact one can imagine the eigendecomposition of a matrix as a way to "summarize" the important things that a matrix, viewed as an operator on a vectorial space, represents.
A small set of properties of eigenvalues of the adjacency matrix of a graph is shown in the following list:
...
When talking about networks, when they are thought to come from some random network model like the Erdos-Renyi model, Watts-Strogats model, Barabasi-Albert or others), all the concepts derived from the spectral theory of matrices are extended to these classes and *spectral densities* of random network models are introduced.
Every random graph model is characterized by its spectral density and this probability distribution is a rather powerful tool to identify different graph models, via some analogies that can be cast between random graph models and physical systems as processors of information.

One of the first observations of this idea is that large networks generated by the same random process are found to have the same spectrum distribution, allowing us to use it as a "fingerprint" of graphs.
In a 2012 paper, [Takahashi](#Takahashi2012) introduced comparison of spectral densities of graph models as the tool for the identification of differences between groups of brain networks. Additionally, they proposed [Takahashi](#Takahashi2012) general methods for model selection and network model parameter estimation, as well as a statistical procedure to test the nullity of divergence between two classes of complex networks.

In these brief notes I'll try to summarize the main results of their paper, lay down some lines of codes to generate some examples [Dehmer2015](#Dehmer2015) and finally discuss pros and cons of this approach and wheter this can be applied to brain networks comparison. 
Finally, I will show some recent results from the paper of [DeDomenico](#DeDomenico2016) that takes a little different approach in the definition of spectral densities and casts a powerful analogy between complex networks and quantum theory, lying down the foundations of something that in the next years will, very probably be part of the basic knowledge in network science.
